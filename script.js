...
function tsvToDeck(tsv){ const lines=tsv.split(/\r?\n/).map(x=>x.trim()).filter(Boolean); const cards=[]; for(const line of lines){ const [hanzi,pinyin,english] = line.split(/\t+/); if(!hanzi||!pinyin||!english) continue; const id=`${hanzi}\n${pinyin}`; cards.push({id, front:hanzi, back:`${pinyin} — ${english}`}); } return cards; } 
...
function makeId(hanzi,pinyin){ return `${hanzi}\n${pinyin}`; } 
